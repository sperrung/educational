...
Пояснение:
В результате группировки по двум полям для каждого месяца у вас должно появиться ещё по две подгруппы с типом действия 
(создание заказа и отмена). В каждой такой подгруппе необходимо посчитать число заказов.*/
